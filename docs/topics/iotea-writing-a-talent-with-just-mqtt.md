...
- Your MQTT Client should be connected the the broker instance, the platform is connected to
  - Subscribe to the following topics:<br>
    ...
    - `talent/<talentId>/events/<callChannelId>/+` (_Topic C_)
...
- `$share/<talentId>/configManager/talents/discover` (_Topic A_)
...

  {
      "id": "<talentId>",
      "config": {
          "aid": "mqtt://localhost:1883",                                             // Identifies the used protocol. Is normally generated by the protocol adapter. For MQTT it's simply the broker URL
...
- `talent/<talentId>/events/<callChannelId>/<deferredCallId>` (_Topic C_)<br>
